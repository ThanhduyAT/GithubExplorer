...
extension UserRepositoryImpl: UserRepository {
    ...
    public func fetchUserList(query: UserListQuery) async throws -> [User] {
        do {
            ...
            let usersData = try await apiClient.fetchUserList(request: UserListRequest.toDTO(query: query))
    ...
    // MARK: - Call API for User List
    
    private func callApi(query: UserListQuery) async throws -> [User] {
        do {
            let usersData = try await apiClient.fetchUserList(request: UserListRequest.toDTO(query: query))
            let users = usersData.compactMap {$0.toDomain()}
            return users
